...
import type { FileSessionRecord, FileSessionStore } from "../file-sessions.js";
...
type RecordWorkspaceFileEventInput = {
...
export type FileSessionRouteDependencies = {
  fileSessions: FileSessionStore;
  recordWorkspaceFileEvent: (workspaceId: string, input: RecordWorkspaceFileEventInput) => unknown;
};
...
const resolveFileSession = (ctx: RequestContext, fileSessions: FileSessionStore, sessionId: string) => {
  ...
  const workspace = ctx.config.workspaces.find((item) => item.id === session.workspaceId);
  if (!workspace) {
...
export function registerFileSessionRoutes(routes: Route[], dependencies: FileSessionRouteDependencies): void {
  const { fileSessions, recordWorkspaceFileEvent } = dependencies;
  ...
  addRoute(routes, "POST", "/files/sessions/:sessionId/renew", "client", async (ctx) => {
    const body = await readJsonBody(ctx.request);
    const ttlMs = parseFileSessionTtlMs((body as Record<string, unknown>).ttlSeconds);
    const { session } = resolveFileSession(ctx, fileSessions, ctx.params.sessionId);
  ...
  addRoute(routes, "GET", "/files/sessions/:sessionId/catalog/snapshot", "client", async (ctx) => {
    const { workspace } = resolveFileSession(ctx, fileSessions, ctx.params.sessionId);
  ...
  addRoute(routes, "POST", "/files/sessions/:sessionId/write-batch", "client", async (ctx) => {
    ...
    ensureWritable(config);
    requireClientScope(ctx, "collaborator");
    const { session, workspace } = resolveFileSession(ctx, fileSessions, ctx.params.sessionId);
    if (!session.canWrite) {

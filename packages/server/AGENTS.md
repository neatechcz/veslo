...
- If you change `packages/server/src`, rebuild the binary with `pnpm --filter openwork-server build:bin`.
- If app behavior depends on a server change, verify against the rebuilt binary, not just the TypeScript sources.
- Use `docs/dev/veslo-server-app-contract.md` and `docs/dev/state-and-config-reference.md` when changing API shape, auth state, persistence, or config precedence.

...
export function buildWindowsWsl2Script(opts: SandboxSpawnOptions, runtime: Wsl2Runtime, opencode: WslOpencodeRuntime): string {
  ...
  return [
    ...
    "  for config_name in opencode.jsonc opencode.json; do",
    "    if [ -f \"$HOST_CONFIG_DIR/$config_name\" ]; then",
    "      cp \"$HOST_CONFIG_DIR/$config_name\" \"$CONFIG_DIR/$config_name\"",
    "    else",
    ...
    "      BWRAP_ARGS+=(--ro-bind \"$HOST_CONFIG_DIR/$config_name\" \"/config/$config_name\")",
    "    fi",
    "  done",
    "fi",

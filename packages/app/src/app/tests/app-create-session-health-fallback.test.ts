...
test("createSessionAndOpen blocks session creation when runtime readiness cannot be proven", () => {
  ...
  assert.match(
    source,
    /if \(!createRuntimeReady\) \{[\s\S]*deps\.recordSendTrace\(\s*"createSessionAndOpen:blocked-runtime-unreachable",[\s\S]*deps\.setError\("Local runtime is not ready yet\."\);[\s\S]*return undefined;[\s\S]*\}/,

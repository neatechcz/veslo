...
test("send preflight records per-step latency trace entries for step 2", () => {
  ...
  assert.match(
    readinessSource,
    /deps\.sendTraceStep\(\s*`\$\{reason\}:runtime-recovery-server-operation`/,
    "server-owned runtime recovery should still be timed from the readiness owner",
  );
});
...
test("skill command resolution uses the scoped send target workspace", () => {
  ...
  assert.match(
    sendWorkflowSource,
    /\(\)\s*=>\s*maybeResolveSkillCommand\(\s*resolvedDraft,\s*sendTraceId,\s*sendTargetWorkspace,?\s*\)/,
...
test("send-time workspace activation preserves scoped browsed conversation state", () => {
  assert.match(
    source,
    /createWorkspaceSessionSnapshots\(\{[\s\S]*selectedSessionId,[\s\S]*resolveSelectedSessionBrowseScope,[\s\S]*saveWorkspaceSnapshot:[\s\S]*sessionStore\.saveWorkspaceSnapshot\(workspaceId\),[\s\S]*loadWorkspaceSnapshot:[\s\S]*sessionStore\.loadWorkspaceSnapshot\(workspaceId\),/,
...
test("active conversation busy state follows scoped selected session workspace", () => {
  assert.match(
    source,
    /const activeConversationBusy = createMemo\(\(\) => \{[\s\S]*const sessionId = activeSessionId\(\);[\s\S]*const scope = sessionId[\s\S]*resolveSelectedSessionBrowseScope\(sessionId\)[\s\S]*const workspaceId =[\s\S]*scope\?\.workspaceId\?\.trim\(\)[\s\S]*currentWorkspaceStoreRef\(\)\?\.activeWorkspaceId\(\)\.trim\(\)[\s\S]*const sessionsForWorkspace =[\s\S]*busySessionByWorkspaceId\(\)\[workspaceId\][\s\S]*sessionIdentityCandidates\(sessionId, scope\)\.some\(\(id\) =>[\s\S]*Boolean\(sessionsForWorkspace\[id\]\)[\s\S]*\);/,
...
test("artifact family workspace root follows scoped selected session", () => {
  assert.match(
    source,
    /resolveArtifactFamilies\(\{[\s\S]*workspaceRoot:[\s\S]*activeSessionId\(\)[\s\S]*resolveSelectedSessionBrowseScope\(activeSessionId\(\)!\)\?\.workspaceRoot[\s\S]*workspaceStore\.activeWorkspaceRoot\(\)\.trim\(\),/,
...
test("conversation read workspace registration dedupes per Veslo client", () => {
  ...
  assert.match(
    conversationServiceSource,
    /if \(!requireLiveOpencodeBaseUrl\) \{[\s\S]*const liveRegistration =[\s\S]*registrationCache\.entries\(\)[\s\S]*const result = await liveRegistration\.promise;[\s\S]*if \(result\.cacheable && result\.id\) return result\.id;/,
  ...
  assert.match(
    conversationServiceSource,
    /if \([\s\S]*!result\.cacheable[\s\S]*registrationCache\.get\(registrationCacheKey\) === registrationFlight[\s\S]*\) \{[\s\S]*registrationCache\.delete\(registrationCacheKey\);/,
...
test("runtime owner gates app-level routing client reads", () => {
  assert.match(
    source,
    /const runtimeOwner = createRuntimeOwner\(\{[\s\S]*routing: workspaceRouting,[\s\S]*\}\);[\s\S]*const runtimeOwnedRouting = createRuntimeOwnedRouting\(\s*workspaceRouting,\s*runtimeOwner,?\s*\);[\s\S]*const routedClient = \(workspaceId\?: string\) =>\s*runtimeOwner\.client\(workspaceId\);/,
    "app should create a runtime-owned routing wrapper next to the runtime owner",
  );
  assert.match(
    source,
    /requiresOrchestratorReadiness: \(workspaceId\) => \{[\s\S]*engineRuntime\(\) !== "veslo-orchestrator"[\s\S]*currentWorkspaceStoreRef\(\)[\s\S]*\.workspaces\(\)[\s\S]*\.find\(\(entry\) => entry\.id === workspaceId\)[\s\S]*workspace\?\.workspaceType === "local";[\s\S]*\}/,
...
test("local sidebar runtime failures fall back to passive conversation read", () => {
  ...
  assert.match(
    conversationServiceSource,
    /const backfillConversationsToVesloReadApi = async[\s\S]*serverClient\.importConversations\(serverWorkspaceId,[\s\S]*deps\.rememberConversationScopesFromSessions\([\s\S]*workspaceId,[\s\S]*directory,[\s\S]*result\.items,[\s\S]*\);/,
    "app should backfill imported live sessions through the Veslo read API registration path",
  );
  assert.match(
    sidebarWorkspaceSessionsSource,
    /setSidebarSessionErrorByWorkspaceId\(\(prev\) =>[\s\S]*\{ \.\.\.prev, \[id\]: null \}[\s\S]*\);/,
...
test("MCP server refresh joins duplicate refreshes for the same workspace context", () => {
  ...
  assert.match(
    source,
    /refreshMcpServers = createMcpServersRefresher\(\{[\s\S]*projectDir: \(\) => workspaceProjectDir\(\),[\s\S]*workspaceType: \(\) => workspaceStore\.activeWorkspaceDisplay\(\)\.workspaceType,[\s\S]*activeRuntimeActivityId: activeVisibleRuntimeActivityId,[\s\S]*scheduleRuntimeStatusRefresh:[\s\S]*mcpConnectionWorkflow\.scheduleMcpRuntimeStatusRefresh,[\s\S]*\}\);/,
...
test("pending permission interval skips active sends and single-client mode covered by SSE", () => {
  assert.match(
    source,
    /const \[visibleRuntimeActivityHold, setVisibleRuntimeActivityHold\] =[\s\S]*createSignal[\s\S]*const holdVisibleRuntimeActivity =[\s\S]*sessionId: string \| null \| undefined,[\s\S]*reason: string,[\s\S]*const token = `run-handoff:\$\{id\}`;[\s\S]*setVisibleRuntimeActivityHold\(\{ sessionId: id, token, expiresAt \}\);[\s\S]*const activeVisibleRuntimeActivityId = \(\) => \{[\s\S]*const sendTraceId = activeSendTraceId\(\)\?\.trim\(\);[\s\S]*if \(sendTraceId\) return sendTraceId;[\s\S]*const sessionId = selectedSessionId\(\)\?\.trim\(\);[\s\S]*const status = statusForSession\(sessionId\);[\s\S]*if \(status === "running" \|\| status === "retry"\) return `run:\$\{sessionId\}`;[\s\S]*const hold = visibleRuntimeActivityHold\(\);[\s\S]*if \(hold\?\.sessionId === sessionId && hold\.expiresAt > Date\.now\(\)\)\s*return hold\.token;/,
...
test("selected transcript reads stay passive while accepted-run recovery owns server start", () => {
  ...
  assert.match(
    source,
    /hydrateLatestSessionFromDb: async \([\s\S]*workspaceId: string,[\s\S]*directory: string,[\s\S]*const result = await listConversationsFromVesloReadApi\([\s\S]*const snapshot = await getTranscriptFromVesloReadApi\(\s*workspaceId,\s*latest\.id,\s*50,\s*directory,?\s*\);/,

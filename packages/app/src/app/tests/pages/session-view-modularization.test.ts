...
test("conversation flow controller owns prompt send orchestration behind grouped dependencies", () => {
  ...
  assert.match(
    sessionSource,
    /const sessionFlowFacade = createSessionViewFlowFacade\(\{ conversationFlow \}\);[\s\S]*const result = await sessionFlowFacade\.handleSendPrompt\(draft, \{[\s\S]*sendNow: options\.sendNow,[\s\S]*sendTraceId: options\.sendTraceId,[\s\S]*source: options\.source,[\s\S]*\}\);[\s\S]*return result;/s,

...
test("after pending draft deletion New session falls back to a fresh private workspace", () => {
  ...
  assert.notEqual(openNewSessionStart, -1, "New session flow should exist in pending draft controller");
  assert.notEqual(openNewSessionEnd, -1, "New session flow end should exist in pending draft controller");
  const openNewSessionSource = pendingDraftControllerSource.slice(openNewSessionStart, openNewSessionEnd);

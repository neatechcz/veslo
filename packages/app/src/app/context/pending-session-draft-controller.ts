...
export function createPendingSessionDraftController(deps: PendingSessionDraftControllerDeps) {
  ...
  const hydrateActivePendingDraft = async () => {
    ...
    try {
      ...
      const loadedPendingDraft = await deps.pendingSessionDraftsGet(matchingPendingDraft.id);
      ...
      restorePendingDraftComposer(storedPendingDraftKey, loadedPendingDraft.draft.composer);
    } catch (error) {

...
function _wsLog(msg: string, data?: unknown) {
  const serializedData = data === undefined ? "" : typeof data === "string" ? data : safeStringify(data);
  const line = `[${new Date().toISOString()}] ${msg}${serializedData ? " " + serializedData : ""}`;
  console.log(line);
  ...
  try {
    void import("../lib/tauri").then((mod) => mod.logUiEvent("workspace", msg, serializedData)).catch(() => {});
  } catch {}
}

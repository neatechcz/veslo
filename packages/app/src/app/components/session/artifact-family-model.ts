...
function isTechnicalArtifactPath(path: string | undefined): boolean {
  ...
  if (normalized.startsWith("prompts/") || normalized.includes("/prompts/")) return true;
  return false;
}

function toFamilyItem(item: VesloSessionArtifactItem): ArtifactFamilyItem | null {
  if (item.family === "files" && isTechnicalArtifactPath(item.path)) {
    return null;
  }
  return {
    id: item.id,
...
export function buildArtifactFamilies(input: BuildArtifactFamiliesInput): ArtifactFamily[] {
  ...
  for (const artifact of input.artifacts ?? []) {
    const item = toFamilyItem(artifact);
    if (!item) continue;
...
function buildLegacyFallbackArtifacts(input: ResolveArtifactFamiliesInput): VesloSessionArtifactItem[] {
  ...
  const pushPath = (rawPath: string, timestamp: number) => {
    ...
    if (!path || isTechnicalArtifactPath(path)) return;
...
export function resolveArtifactFamilies(input: ResolveArtifactFamiliesInput): ArtifactFamily[] {
  ...
  if (input.preferServerArtifacts === true && input.serverArtifacts) {
    return buildArtifactFamilies({ artifacts: serverArtifacts });
  }
  if (serverArtifacts.length > 0) {
    return buildArtifactFamilies({ artifacts: serverArtifacts });
  }
  return buildArtifactFamilies({ artifacts: buildLegacyFallbackArtifacts(input) });
}

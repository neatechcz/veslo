...
export type ScheduledTasksViewProps = {
  ...
  legacyScheduledJobs: ScheduledJob[];
  source: "local" | "remote";
  sourceReady: boolean;
  status: string | null;
  busy: boolean;
  ...
  deleteLegacyJob: (name: string) => Promise<void> | void;
  isWindows: boolean;
...
const LegacyJobCard = (props: {
  ...
  return (
    <div class="rounded-2xl border border-gray-4 bg-gray-1 p-5 shadow-sm">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0">
          <h3 class="truncate text-sm font-semibold text-gray-12">{props.job.name}</h3>
          <div class="mt-1 font-mono text-xs text-gray-9">{props.job.schedule}</div>
        </div>
        <button
          type="button"
          onClick={props.onDelete}
          disabled={props.busy}
          class={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${
            props.busy ? "border-gray-5 text-gray-8" : "border-red-6 text-red-10 hover:bg-red-3"
          }`}
        >
          <Trash2 size={12} />
          {tr("scheduled.delete")}
      ...
      <div class="mt-3 grid gap-3 md:grid-cols-2">
        <div class="rounded-xl border border-gray-4 bg-gray-2/60 px-3 py-3">
          <div class="text-[10px] uppercase text-gray-8">{tr("scheduled.prompt_label")}</div>
          <div class="mt-1 line-clamp-3 break-words text-sm text-gray-12">{props.job.run?.prompt ?? props.job.prompt ?? tr("scheduled.no_prompt_found")}</div>
        </div>
        <div class="rounded-xl border border-gray-4 bg-gray-2/60 px-3 py-3 text-xs text-gray-9">
          <div class="flex items-center gap-2">
            <Terminal size={14} class="text-gray-8" />
            <span class="font-mono break-all text-gray-12">{props.job.workdir ?? tr("scheduled.default")}</span>
          </div>
          <div class="mt-2">{tr("scheduled.last_run")} {toRelative(props.job.lastRunAt, locale())}</div>
        </div>
      </div>
    </div>
  );
...
export default function ScheduledTasksView(props: ScheduledTasksViewProps) {
  ...
  const [legacyDeleteTarget, setLegacyDeleteTarget] = createSignal<ScheduledJob | null>(null);
  ...
  const confirmLegacyDelete = async () => {
    const target = legacyDeleteTarget();
    if (!target) return;
    await legacyDeleteAction.execute(async () => {
      await props.deleteLegacyJob(target.slug);
  ...
  return (
    <section class="space-y-8">
        ...
        <div class="rounded-2xl border border-gray-5 bg-gray-2/70 px-5 py-5 shadow-sm">
          <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex items-start gap-3">
              ...
              <div>
                <div class="text-sm font-semibold text-gray-12">{tr("scheduled.legacy_scheduler_title")}</div>
                <div class="mt-1 text-xs text-gray-9">{tr("scheduled.legacy_scheduler_hint")}</div>
              </div>
            </div>
            <div class="flex flex-wrap items-center gap-2">
              <Button variant="secondary" onClick={handleInstallScheduler} disabled={!props.canEditPlugins || installingScheduler()}>
                {installingScheduler() ? tr("scheduled.installing") : tr("scheduled.install_scheduler")}
              </Button>
              <Button variant="outline" onClick={() => void props.reloadWorkspaceEngine()} disabled={!props.canReloadWorkspace || props.reloadBusy || !props.schedulerInstalled}>
      ...
      <Show when={props.status || createAction.error() || runAction.error() || deleteAction.error() || legacyDeleteAction.error()}>
        <div class="rounded-xl border border-red-7/40 bg-red-3/60 px-5 py-4 text-sm text-red-11">
          {props.status ?? createAction.error() ?? runAction.error() ?? deleteAction.error() ?? legacyDeleteAction.error()}
        </div>
      </Show>
      ...
      <Show when={props.legacyScheduledJobs.length > 0}>
        <section class="space-y-4 border-t border-gray-4 pt-6">
          <div>
            <h3 class="text-sm font-semibold text-gray-12">{tr("scheduled.legacy_jobs_title")}</h3>
            <p class="mt-1 text-xs text-gray-9">{tr("scheduled.legacy_jobs_description")}</p>
          </div>
          <div class="grid w-full grid-cols-1 gap-4">
            <For each={props.legacyScheduledJobs}>
              {(job) => (
                <LegacyJobCard
                  job={job}
                  busy={props.busy || legacyDeleteAction.busy()}
                  onDelete={() => setLegacyDeleteTarget(job)}
                />
              )}
            </For>
          </div>
      ...
      <Show when={legacyDeleteTarget()}>
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/20 p-4 backdrop-blur-sm">
          <div class="w-full max-w-md overflow-hidden rounded-2xl border border-gray-6 bg-gray-1 shadow-2xl">
            <div class="space-y-4 p-6">
              <h3 class="text-lg font-semibold text-gray-12">{tr("scheduled.delete_title")}</h3>
              <p class="text-sm text-gray-9">{tr("scheduled.delete_desc_local")}</p>
              <div class="rounded-xl border border-gray-6 bg-gray-2 p-3 font-mono text-xs text-gray-9 break-all">
                {legacyDeleteTarget()?.name}
              </div>
              <div class="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setLegacyDeleteTarget(null)} disabled={legacyDeleteAction.busy()}>
                  {tr("scheduled.cancel")}
                </Button>
                <Button variant="danger" onClick={confirmLegacyDelete} disabled={legacyDeleteAction.busy()}>
                  {legacyDeleteAction.busy() ? tr("scheduled.deleting") : tr("scheduled.delete")}
                </Button>
              </div>
            </div>
          </div>

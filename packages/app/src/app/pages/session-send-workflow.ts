...
import type { VesloConversationSubmitRequest, VesloConversationSubmitResult, VesloServerClient } from "../lib/veslo-server";
import { VesloServerError } from "../lib/veslo-server";
import { documentRuntimeTaskBlockReason, type DocumentRuntimeFormat, type DocumentRuntimeStatusPayload } from "../lib/document-runtime";
import type { DisplayedConversationGuard, SendTargetWorkspaceScope } from "../context/workspace-session-selection";
...
import type { ConversationServerSubmitPreflightError, ConversationAbortTarget, ConversationSendPreflightContext } from "../context/conversation-service";
...
import type { SidebarActivityTokenHandle, SidebarActivityTokenScope } from "../context/sidebar-session-activity-token";
...
import type { Client, ComposerDraft, ComposerPart, ModelRef, PendingSidebarSessionMetadata, ProviderListItem, View, WorkspaceDisplay } from "../types";
...
const isMissingLiveBindingPreflightError = (error: ConversationServerSubmitPreflightError) =>
  error.code === "local_live_binding_unavailable" && error.httpAttempted === false;

const isKnownServerSubmitResponseError = (error: unknown): error is VesloServerError =>
  error instanceof VesloServerError && error.status >= 400 && error.status < 500;

function conversationSubmitDraftFromComposerDraft(draft: ComposerDraft, stagedAttachments: StagedSessionAttachment[] = []): ConversationSubmitDraftInput {
  return {
    mode: draft.mode,
    text: draft.text,
    ...
    attachments: draft.attachments.map((attachment, index) => ({
      ...
      dataUrl: attachment.kind === "image" || !stagedAttachments[index] ? attachment.dataUrl : null,
...
export function documentRuntimeBlockReasonForSkillCommand(status: DocumentRuntimeStatusPayload | null | undefined, skillName: string): string | null {
...
export type SessionSendPreflightContext = SendRuntimePreflightContext & ConversationSendPreflightContext<VesloServerClient>;
...
export type SessionSendWorkflowOptions = {
  ...
  migrateSidebarActivityToken?: (handle: SidebarActivityTokenHandle | null | undefined, scope: SidebarActivityTokenScope) => boolean;
  promoteSidebarActivityToken?: (handle: SidebarActivityTokenHandle | null | undefined, runId: string | null | undefined) => boolean;
  ...
  abortConversationFromVesloWriteApi: (
    ...
    | undefined
  >;
  abortSessionTyped: (client: Client, sessionId: string, options?: { directory?: string }) => Promise<unknown>;
  ...
  engineReady: () => boolean;
  ensureLocalRuntimeReachableForSend: (event: "sendPrompt", preflight: SessionSendPreflightContext) => Promise<boolean>;
  ensureServerOwnedSubmitTransportReady: (event: "sendPrompt", preflight: SessionSendPreflightContext) => Promise<boolean>;
  ...
  finishPerf: (enabled: boolean, scope: string, event: string, startedAt: number, payload?: Record<string, unknown>) => void;
  ...
  recordPerfLog: (enabled: boolean, scope: string, event: string, payload?: Record<string, unknown>) => void;
  ...
  resolveRuntimeSandboxStateForTarget: (targetWorkspace?: SendRuntimePreflightTargetWorkspace | null) => unknown;
  ...
  sendTraceStep: <T>(event: string, run: () => Promise<T>, payload?: Record<string, unknown>) => Promise<T>;
  ...
  stageServerSubmitAttachments: (draft: ComposerDraft, sessionId: string, preflight?: SessionSendPreflightContext) => Promise<StagedSessionAttachment[]>;
...
function sessionSubmitResultFromConversationSubmit(result: ConversationSubmitTerminalResult): SessionSubmitResult {
...
function sendBoundaryValidationOptions(deps: SendBoundaryValidationRuntimeDeps, options: SendBoundaryValidationRuntimeOptions) {
  return {
    ...options,
...
export function createSessionSendWorkflow(deps: SessionSendWorkflowOptions): SessionSendWorkflow {
  async function maybeResolveSkillCommand(
    ...
    if (deps.vesloServerStatus() !== "connected" || !vesloClient || !workspaceId || typeof vesloClient.resolveSkill !== "function") {
    ...
    try {
      const targetWorkspaceType = targetWorkspaceId
        ? (deps.workspace.workspaces().find((workspace) => workspace.id === targetWorkspaceId)?.workspaceType ??
          (deps.workspace.activeWorkspaceId().trim() === targetWorkspaceId ? deps.workspace.activeWorkspaceDisplay().workspaceType : undefined))
      ...
      const resolution = await deps.sendTraceStep(
        ...
        () =>
          (
            vesloClient as {
              resolveSkill: (workspaceId: string, payload: { text: string; includeGlobal?: boolean }) => Promise<{ match?: { name?: string | null } | null }>;
            }
      ...
      const commandDirectory = targetWorkspace?.directory?.trim() || targetWorkspace?.workspaceRoot?.trim() || "";
      ...
      const matchedCommand = commands.find((entry) => entry.name === matchedName && entry.source === "skill");
      if (!matchedCommand) {
  ...
  async function sendPrompt(draft: ComposerDraft, options: SessionSendWorkflowSendOptions): Promise<SessionSubmitResult> {
    ...
    const selectedSessionScopeForSend = selectedSessionCandidate ? deps.resolveSelectedSessionBrowseScope(selectedSessionCandidate) : null;
    ...
    let releaseSendPromptInFlight: (() => void) | null = deps.startSendPromptInFlight?.() ?? null;
    ...
    const startSendPromptBusy = (event: Extract<SessionFlowProgressEvent, { type: "runtime.connecting" | "conversation.running" }>) => {
    ...
    try {
      ...
      const shouldUseServerSubmitBeforeFrontendSkillResolution = Boolean(deps.submitConversationFromVesloWriteApi);
      ...
      const submitExistingSessionWithServer = async (): Promise<SessionSubmitResult | null> => {
        ...
        const directory = sendTargetWorkspace?.directory?.trim() || sendTargetWorkspace?.workspaceRoot?.trim() || "";
        ...
        const submitRequest: VesloConversationSubmitRequest = {
          ...
          options: {
            ...
            submitQueuePolicy:
              sendCorrelation.origin === "session:send-now" ? "send-now" : sendCorrelation.origin === "session:queue-drain" ? "server-queue-only" : "normal",
          },
        };
        ...
        const provisionalRun = conversationId
          ...
          : null;
        const provisionalArmed = provisionalRun ? deps.armConversationRunProvisional?.(provisionalRun) === true : false;
        ...
        const failPreflight = (error: unknown) => {
          ...
          deps.recordSendTrace("sendPrompt:server-submit-existing:preflight-error", {
            ...
            origin: sendCorrelation.origin,
            message,
            preflightCode: isConversationServerSubmitPreflightError(error) ? (error.code ?? null) : null,
            httpAttempted: isConversationServerSubmitPreflightError(error) ? error.httpAttempted === true : null,
          });
          reportServerSubmitError(message);
          return sessionSubmitFailedResult({
        ...
        const failKnownServerResponse = (error: VesloServerError) => {
          ...
          const message =
            error.code === "terminal_handoff_recovery_required"
          ...
          deps.recordSendTrace("sendPrompt:server-submit-existing:server-response-error", {
            traceId: sendTraceId,
            sessionID: existingSessionId,
            clientMessageId: sendCorrelation.clientMessageId,
            origin: sendCorrelation.origin,
        ...
        try {
          result = await deps.sendTraceStep(
            ...
            () => submitConversation(workspaceId, directory, submitRequestValidation.value, sendPreflight),
            {
              traceId: sendTraceId,
              sessionID: existingSessionId,
              workspaceId,
          ...
          if (isConversationServerSubmitPreflightError(firstError)) {
            if (!isMissingLiveBindingPreflightError(firstError)) return failPreflight(firstError);

            sendPreflight.forceRecovery = true;
            sendPreflight.runtimeHealthOk = false;
            sendPreflight.enginePrepared = false;
            deps.recordSendTrace("sendPrompt:server-submit-existing:missing-live-binding-recovery:start", {
              traceId: sendTraceId,
              sessionID: existingSessionId,
              workspaceId,
              clientMessageId: sendCorrelation.clientMessageId,
            ...
            let recovered = false;
            try {
              recovered = await deps.ensureServerOwnedSubmitTransportReady("sendPrompt", sendPreflight);
            } catch (recoveryError) {
              deps.recordSendTrace("sendPrompt:server-submit-existing:missing-live-binding-recovery:error", {
                traceId: sendTraceId,
                sessionID: existingSessionId,
                workspaceId,
                clientMessageId: sendCorrelation.clientMessageId,
                origin: sendCorrelation.origin,
                errorType: recoveryError instanceof Error ? recoveryError.name : "unknown",
              });
            }
            deps.recordSendTrace("sendPrompt:server-submit-existing:missing-live-binding-recovery:end", {
              traceId: sendTraceId,
              sessionID: existingSessionId,
              workspaceId,
              clientMessageId: sendCorrelation.clientMessageId,
            ...
            try {
              ...
              if (isConversationServerSubmitPreflightError(retryError)) return failPreflight(retryError);
              if (isKnownServerSubmitResponseError(retryError)) return failKnownServerResponse(retryError);
          ...
          if (transportErrorObserved) {
            ...
            deps.recordSendTrace("sendPrompt:server-submit-existing:replay-after-transport-error", {
              ...
              clientMessageId: sendCorrelation.clientMessageId,
              origin: sendCorrelation.origin,
              message: deps.messageFromUnknownError(errorForTransportReplay),
            });
            try {
              result = await deps.sendTraceStep(
              ...
              if (isKnownServerSubmitResponseError(error)) return failKnownServerResponse(error);
              disposeProvisional();
              if (commandMessageIDToClear) deps.sessionStoreClearCommandDisplay(commandMessageIDToClear);
              const message = deps.messageFromUnknownError(error);
              deps.finishPerf(perfEnabled, "session.prompt", "error", startedAt, {
        ...
        if (sidebarActivityToken) {
          ...
          sidebarActivityTokenCommitted =
            deps.promoteSidebarActivityToken?.(sidebarActivityToken, result.status === "submitted" ? result.runId : result.reservedRunId) ?? false;
        }
        ...
        if (sendTargetStillDisplayed()) {
          ...
          deps.recordSendTrace("sendPrompt:server-submit-existing:ui-effects-skipped", {
            traceId: sendTraceId,
            sessionID: existingSessionId,
            clientMessageId: sendCorrelation.clientMessageId,
            origin: sendCorrelation.origin,
      ...
      const shouldUseServerSubmitForFirstSession = Boolean(!sessionID && deps.submitConversationFromVesloWriteApi);
      if (!shouldUseServerSubmitForFirstSession) {
        const message = "Server-owned conversation submit is required for this send path.";
        deps.recordSendTrace("sendPrompt:blocked-server-submit-unavailable", {
          traceId: sendTraceId,
          ...
          () =>
            deps.createSessionAndOpen(initialSessionTitle, {
              ...
              onSubmitResult: (result) => {
                if (result.status === "submitted" || result.status === "queued" || result.status === "failed" || result.status === "blocked") {
      ...
      if (!sessionID && serverFirstSubmitResult && (serverFirstSubmitResult.status === "failed" || serverFirstSubmitResult.status === "blocked")) {
      ...
      if (serverFirstSubmitResult) {
          ...
          if (serverFirstSubmitResult.status === "submitted" || serverFirstSubmitResult.status === "queued") {
            sidebarActivityTokenCommitted =
              deps.promoteSidebarActivityToken?.(
                sidebarActivityToken,
                serverFirstSubmitResult.status === "submitted" ? serverFirstSubmitResult.runId : serverFirstSubmitResult.reservedRunId,
              ) ?? false;
          }
        }
        ...
        deps.recordSendTrace("sendPrompt:server-submit-first-success", {
          ...
          origin: sendCorrelation.origin,
          status: serverFirstSubmitResult.status,
          runId: serverFirstSubmitResult.status === "submitted" ? serverFirstSubmitResult.runId : serverFirstSubmitResult.reservedRunId,
  ...
  async function abortSession(sessionID?: string, target?: ConversationAbortTarget) {
    ...
    const explicitAbortWorkspaceId =
      target?.workspaceId?.trim() || selectedAbortScope?.workspaceId?.trim() || (scope.hasConversationScope ? scope.workspaceId?.trim() : "") || "";
    ...
    const unknownAbort = (message: string): SessionAbortResult => ({
      kind: "unknown",
      message,
    });

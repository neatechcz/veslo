...
import { isConversationServerSubmitPreflightError } from "../context/conversation-service";
import type {
...
import { type ComposerDraftStateCommands } from "./session-composer-drafts";
import type {
  Client,
  ComposerDraft,
  ComposerPart,
...
type ConversationSubmitOptionsInput = NonNullable<
  VesloConversationSubmitRequest["options"]
>;

const isMissingLiveBindingPreflightError = (
  error: ConversationServerSubmitPreflightError,
) =>
  error.code === "local_live_binding_unavailable" &&
  error.httpAttempted === false;

const isKnownServerSubmitResponseError = (
  error: unknown,
): error is VesloServerError =>
  error instanceof VesloServerError && error.status >= 400 && error.status < 500;
...
function conversationSubmitDraftFromComposerDraft(
  ...
  return {
    ...
    attachments: draft.attachments.map((attachment, index) => ({
      ...
      dataUrl:
        attachment.kind === "image" || !stagedAttachments[index]
          ? attachment.dataUrl
          : null,
...
export function documentRuntimeFormatForSkillCommand(
  skillName: string,
): DocumentRuntimeFormat | null {
  const key = skillName.trim();
  return Object.prototype.hasOwnProperty.call(
    DOCUMENT_RUNTIME_FORMAT_BY_SKILL_NAME,
    key,
  )
    ? DOCUMENT_RUNTIME_FORMAT_BY_SKILL_NAME[
        key as keyof typeof DOCUMENT_RUNTIME_FORMAT_BY_SKILL_NAME
      ]
    : null;
}
...
type NormalizedSessionSendCorrelation = ReturnType<
  typeof normalizeSessionSendCorrelation
>;

type StartSendPromptBusy = (
  event: Extract<
    SessionFlowProgressEvent,
    { type: "runtime.connecting" | "conversation.running" }
  >,
) => void;

export type SessionSendPreflightContext = SendRuntimePreflightContext &
...
export type ConversationRunCompatibilityBridgeSubmitInput = {
  ...
  consumePendingDraftAfterAcceptedSend: (
    clearDisplayedPendingDraftState: boolean,
  ) => Promise<void>;
  draft: ComposerDraft;
...
export type ConversationRunCompatibilityBridge = {
  prepare: (
    input: ConversationRunCompatibilityBridgePrepareInput,
  ) => Promise<boolean>;
  submit: (
    input: ConversationRunCompatibilityBridgeSubmitInput,
  ) => Promise<boolean>;
};
...
export type SessionSendWorkflowOptions = {
  admitAcceptedConversationRun: (
    input: AcceptedConversationRunInput,
  ) => boolean | void;
  ...
  beginSidebarActivityToken?: (
    scope: SidebarActivityTokenScope,
  ) => SidebarActivityTokenHandle | null;
  ...
  displayedConversationStillMatches: (
    guard: DisplayedConversationGuard,
  ) => boolean;
  engineReady: () => boolean;
  ...
  listCommands: (scope?: {
    workspaceId?: string | null;
    directory?: string | null;
  }) => Promise<SessionSendWorkflowCommand[]>;
  emitLiveTranscriptPolicyEvent: (event: LiveTranscriptReadPolicyEvent) => void;
  ...
  registerPendingSidebarSession: (
    pendingSession: PendingSidebarSessionMetadata,
  ) => void;
  ...
  resolveSendPromptBusyOwnership: (input: {
    sessionId: string | null | undefined;
  }) => { ownsBusy: boolean };
  ...
  sessionStoreAppendSessionErrorTurn: (
    sessionId: string,
    message: string,
  ) => void;
  sessionStoreClearCommandDisplay: (messageId: string) => void;
...
function sessionSubmitResultFromConversationSubmit(
  ...
  const failedQueueItemId =
    "queueItemId" in result ? result.queueItemId : undefined;
  const failedReservedRunId =
    "reservedRunId" in result ? result.reservedRunId : undefined;
...
export function createConversationRunCompatibilityBridge(
  ...
  const prepare = async (
    ...
    const sendRuntimeWorkspaceId =
      input.sendTargetWorkspace?.workspaceId ??
    ...
    const prepareInputValidation = validateConversationRunBridgePrepareInput(
      ...
      sendBoundaryValidationOptions(deps, {
        event:
          "sendPrompt:conversation-run-bridge-prepare-input:validation-failed",
        traceId: input.traceId,
        context: {
    ...
    const sendRuntimeReady = deps.isWorkspaceRuntimeReady(
      sendRuntimeWorkspaceId,
    );
    if (!sendRuntimeReady) {
    ...
    const sendRuntimePreparation = await deps.prepareSendRuntimeForSend(
      "sendPrompt",
      input.sendPreflight,
    );
    const sendRuntimePreparationValidation =
      validateSendRuntimePreparationResult(
        sendRuntimePreparation,
        sendBoundaryValidationOptions(deps, {
    ...
    input.sendPreflight.effectiveSandbox =
      deps.resolveRuntimeSandboxStateForTarget(input.sendTargetWorkspace);
  ...
  const submit = async (
    ...
    const submitInputValidation = validateConversationRunBridgeSubmitInput(
      ...
      sendBoundaryValidationOptions(deps, {
        event:
          "sendPrompt:conversation-run-bridge-submit-input:validation-failed",
        traceId: input.traceId,
        context: {
    ...
    const model =
      input.modelOverride ?? deps.modelForSession(materializedSessionID);
    ...
    try {
      let stagedAttachments = await deps.sendTraceStep(
        ...
        () =>
          deps.stageAttachmentsIntoSessionDirectory(
            resolvedDraft,
      ...
      const stagedAttachmentsValidation = validateStagedSessionAttachments(
        stagedAttachments,
        sendBoundaryValidationOptions(deps, {
      ...
      const routedDraftValidation = validateRoutedComposerDraftResult(
        routedDraft,
        sendBoundaryValidationOptions(deps, {
          event:
            "sendPrompt:staged-attachment-routing-result:validation-failed",
          traceId: input.traceId,
          context: {
    ...
    const visibleParts = visible.reduce(
    ...
    try {
      ...
      const sessionDirOverride =
        deps.sessionDirectoryOverrideById()[materializedSessionID] ?? undefined;
      let localRuntimeRetryUsed = false;
      const runConversationOrFail = async (
        runInput: VesloConversationRunInput,
      ) => {
        const scope = deps.resolveSelectedSessionBrowseScope(
          materializedSessionID,
        );
        ...
        const submitConversationRun = async () =>
          deps.submitConversationRunViaVesloWriteApi(
            materializedSessionID,
        ...
        try {
          ...
          throw new Error(
            "Conversation service is unavailable for this session.",
          );
        } catch (error) {
          ...
          if (
            recoverable &&
            !localRuntimeRetryUsed &&
            input.sendTargetStillDisplayed()
          ) {
            ...
            deps.recordSendTrace(
              "sendPrompt:conversation-run-runtime-recovery-start",
              {
                traceId: input.traceId,
                sessionID,
                kind: runInput.kind,
            ...
            const recovery = await deps.prepareSendRuntimeForSend(
              "sendPrompt",
              input.sendPreflight,
            );
            const recoveryValidation = validateSendRuntimePreparationResult(
              recovery,
              sendBoundaryValidationOptions(deps, {
            ...
            deps.recordSendTrace(
              "sendPrompt:conversation-run-runtime-recovery-result",
              {
                traceId: input.traceId,
                sessionID,
                kind: runInput.kind,
      ...
      if (resolvedDraft.mode === "shell") {
        ...
        deps.sessionStoreSetCommandDisplay(
          commandMessageID,
          command.name,
          command.arguments,
        );
      ...
      await input.consumePendingDraftAfterAcceptedSend(
        input.sendTargetStillDisplayed(),
      );
...
export function createSessionSendWorkflow(
  deps: SessionSendWorkflowOptions,
): SessionSendWorkflow {
  const conversationRunCompatibilityBridge =
    deps.conversationRunCompatibilityBridge ?? null;
  ...
  async function maybeResolveSkillCommand(
    ...
    if (
      ...
      deps.recordSendTrace(
        "maybeResolveSkillCommand:skipped-veslo-server-unavailable",
        {
          ...(tracePayload ?? {}),
    ...
    try {
      ...
      if (!matchedCommand) {
        deps.recordSendTrace(
          "maybeResolveSkillCommand:matched-skill-command-missing",
          {
            ...(tracePayload ?? {}),
            matchedName,
      ...
      const documentRuntimeBlockedReason = deps.documentRuntimeStatus
        ? documentRuntimeBlockReasonForSkillCommand(
            deps.documentRuntimeStatus(),
            matchedName,
          )
        : null;
      if (documentRuntimeBlockedReason) {
        deps.recordSendTrace(
          "maybeResolveSkillCommand:blocked-document-runtime",
          {
            ...(tracePayload ?? {}),
            matchedName,
  ...
  async function sendPrompt(
    ...
    if (!sendCorrelation.clientMessageId) {
      ...
      return sessionSubmitBlockedResult({
        ...
        message:
          "Cannot send this prompt because its client message id is missing.",
      });
    }
    ...
    const selectedSessionScopeForSend = selectedSessionCandidate
      ...
      : null;
    const selectedSessionScopeWorkspaceId =
      selectedSessionScopeForSend?.workspaceId?.trim() ?? "";
    ...
    const selectedRealSessionId =
      deps.isPendingSessionInstanceKey(selectedSessionCandidate) ||
    ...
    const pendingSidebarTargetWorkspace =
    ...
    let sendTargetWorkspace =
      pendingSidebarTargetWorkspace ??
      deps.resolveSendTargetWorkspaceScope(sessionID);
    sendPreflight.targetWorkspace = sendTargetWorkspace;
    sendPreflight.effectiveSandbox =
      deps.resolveRuntimeSandboxStateForTarget(sendTargetWorkspace);
    ...
    const sendPromptBusyOwnership = deps.resolveSendPromptBusyOwnership({
      sessionId: sessionID,
    });
    ...
    try {
      ...
      const composerResolvedText = (
        composerDraftSnapshot.resolvedText ?? composerDraftSnapshot.text
      ).trim();
      let resolvedDraft: ComposerDraft = draft ?? {
        mode: composerDraftSnapshot.mode,
        parts: composerDraftSnapshot.parts.length
          ? composerDraftSnapshot.parts
          : composerText
            ? [{ type: "text", text: composerText } as ComposerPart]
            : [],
      ...
      if (shouldUseServerSubmitBeforeFrontendSkillResolution) {
        deps.recordSendTrace(
          "sendPrompt:maybe-resolve-skill-command:server-owned-skip",
          {
            traceId: sendTraceId,
            mode: resolvedDraft.mode,
      ...
      if (deps.submitConversationFromVesloWriteApi) {
        ...
        const tokenSessionId =
          sessionID?.trim() || pendingSidebarSession?.id?.trim() || "";
        if (tokenWorkspaceId && tokenSessionId) {
          sidebarActivityToken =
            deps.beginSidebarActivityToken?.({
      ...
      const compactCommand =
        resolvedDraft.command?.name === "compact" || compactShortcut;
      const commandName = compactCommand
        ? "compact"
        : (resolvedDraft.command?.name ?? null);
      ...
      const hadExistingSessionBeforeMaterialization = Boolean(
        sessionID?.trim(),
      );
      const submitExistingSessionWithServer =
        async (): Promise<SessionSubmitResult | null> => {
          const submitConversation = deps.submitConversationFromVesloWriteApi;
          const existingSessionId = sessionID?.trim() || "";
          if (
            !submitConversation ||
            !existingSessionId ||
            !hadExistingSessionBeforeMaterialization
          )
            return null;

          const displayedConversationGuard =
            deps.captureDisplayedConversationGuard(existingSessionId);
          const displayedUiScopeToken = deps.activeUiScopeToken();
          const sendTargetStillDisplayed = () =>
            deps.displayedConversationStillMatches(
              displayedConversationGuard,
          ...
          const reportServerSubmitError = (message: string) => {
            ...
            deps.sessionStoreAppendSessionErrorTurn(
              existingSessionId,
              hintedMessage,
            );
          };
          ...
          if (!workspaceId || !directory) {
            const message =
              "Server-owned conversation submit is missing a workspace or directory for this local session.";
            deps.recordSendTrace(
              "sendPrompt:server-submit-existing-missing-target",
              {
                traceId: sendTraceId,
          ...
          const scope =
            deps.resolveSelectedSessionBrowseScope(existingSessionId);
          const conversationId = scope?.conversationId?.trim() || null;
          const opencodeSessionId =
            scope?.opencodeSessionId?.trim() || existingSessionId;
          const model =
            options.modelOverride ?? deps.modelForSession(existingSessionId);
          ...
          if (resolvedDraft.attachments.length > 0) {
            try {
              ...
              stagedAttachments = await deps.sendTraceStep(
                "sendPrompt:server-submit-existing-stage-attachments",
                () =>
                  deps.stageServerSubmitAttachments(
                    resolvedDraft,
                    existingSessionId,
                    sendPreflight,
                  ),
              ...
              const stagedAttachmentsValidation =
                validateStagedSessionAttachments(
                  stagedAttachments,
                  sendBoundaryValidationOptions(deps, {
                    event:
                      "sendPrompt:server-submit-existing-stage-attachments-result:validation-failed",
                    traceId: sendTraceId,
                    context: {
              ...
              stagedAttachments = stagedAttachmentsValidation.value;
              deps.recordSendTrace(
                "sendPrompt:server-submit-existing-stage-attachments-done",
                {
                  traceId: sendTraceId,
                  sessionID: existingSessionId,
                  clientMessageId: sendCorrelation.clientMessageId,
              ...
              deps.recordSendTrace(
                "sendPrompt:server-submit-existing-stage-attachments-error",
                {
                  traceId: sendTraceId,
                  sessionID: existingSessionId,
                  clientMessageId: sendCorrelation.clientMessageId,
          ...
          const submitRequest: VesloConversationSubmitRequest = {
            ...
            draft: conversationSubmitDraftFromComposerDraft(
              resolvedDraft,
              stagedAttachments,
            ),
            options: {
              ...
              ...(options.modelOverride
                ...
                : {}),
              submitQueuePolicy:
                sendCorrelation.origin === "session:send-now"
                  ? "send-now"
          ...
          const disposeProvisional = () => {
            if (provisionalArmed && provisionalRun)
              deps.disposeConversationRunProvisional?.(provisionalRun);
          };
          const failPreflight = (error: unknown) => {
            disposeProvisional();
            if (commandMessageIDToClear)
              deps.sessionStoreClearCommandDisplay(commandMessageIDToClear);
            const message = deps.messageFromUnknownError(error);
            ...
            deps.recordSendTrace(
              "sendPrompt:server-submit-existing:preflight-error",
              {
                ...
                origin: sendCorrelation.origin,
                message,
                preflightCode: isConversationServerSubmitPreflightError(error)
                  ? (error.code ?? null)
                  : null,
                httpAttempted: isConversationServerSubmitPreflightError(error)
                  ? error.httpAttempted === true
                  : null,
              },
            );
            reportServerSubmitError(message);
          ...
          const failKnownServerResponse = (error: VesloServerError) => {
            disposeProvisional();
            if (commandMessageIDToClear)
              deps.sessionStoreClearCommandDisplay(commandMessageIDToClear);
            const message = deps.messageFromUnknownError(error);
            ...
            deps.recordSendTrace(
              "sendPrompt:server-submit-existing:server-response-error",
              {
                traceId: sendTraceId,
                sessionID: existingSessionId,
                clientMessageId: sendCorrelation.clientMessageId,
            ...
            reportServerSubmitError(message);
            return sessionSubmitFailedResult({
              code: "server_submit_response_failed",
              message,
              draftDisposition: "keep",
            });
          };
          ...
          try {
            ...
            if (isConversationServerSubmitPreflightError(firstError)) {
              ...
              try {
                ...
                deps.recordSendTrace(
                  "sendPrompt:server-submit-existing:missing-live-binding-recovery:error",
                  {
                    traceId: sendTraceId,
                    sessionID: existingSessionId,
                    workspaceId,
              ...
              deps.recordSendTrace(
                "sendPrompt:server-submit-existing:missing-live-binding-recovery:end",
                {
                  traceId: sendTraceId,
                  sessionID: existingSessionId,
                  workspaceId,
              ...
              if (!recovered) return failPreflight(firstError);

              try {
                result = await deps.sendTraceStep(
                  "sendPrompt:server-submit-existing:retry-after-missing-live-binding",
                  () =>
                    submitConversation(
                      workspaceId,
                      directory,
                ...
                if (isConversationServerSubmitPreflightError(retryError))
                  return failPreflight(retryError);
                if (isKnownServerSubmitResponseError(retryError))
                  return failKnownServerResponse(retryError);
                errorForTransportReplay = retryError;
                transportErrorObserved = true;
              }
            } else if (isKnownServerSubmitResponseError(firstError)) {
              return failKnownServerResponse(firstError);
            } else {
              transportErrorObserved = true;
            }
            if (transportErrorObserved) {
              ...
              deps.recordSendTrace(
                "sendPrompt:server-submit-existing:replay-after-transport-error",
                {
                  traceId: sendTraceId,
                  sessionID: existingSessionId,
                  clientMessageId: sendCorrelation.clientMessageId,
                  origin: sendCorrelation.origin,
                  message: deps.messageFromUnknownError(
                    errorForTransportReplay,
                  ),
                },
              );
              try {
                ...
                if (isKnownServerSubmitResponseError(error))
                  return failKnownServerResponse(error);
                disposeProvisional();
                if (commandMessageIDToClear)
                  deps.sessionStoreClearCommandDisplay(commandMessageIDToClear);
                const message = deps.messageFromUnknownError(error);
                ...
                deps.recordSendTrace(
                  "sendPrompt:server-submit-existing:outcome-unknown",
                  {
                    traceId: sendTraceId,
                    sessionID: existingSessionId,
                    clientMessageId: sendCorrelation.clientMessageId,
          ...
          if (!result) {
            ...
            deps.recordSendTrace(
              "sendPrompt:server-submit-existing-unavailable",
              {
                traceId: sendTraceId,
                sessionID: existingSessionId,
                workspaceId,
          ...
          const resultValidation = validateConversationSubmitTerminalResult(
            result,
            sendBoundaryValidationOptions(deps, {
              event:
          ...
          if (result.status === "blocked" || result.status === "failed") {
            ...
            deps.recordSendTrace(
              `sendPrompt:server-submit-existing-${result.status}`,
              {
                traceId: sendTraceId,
                sessionID: existingSessionId,
                clientMessageId: sendCorrelation.clientMessageId,
          ...
          if (sidebarActivityToken) {
            ...
            sidebarActivityTokenCommitted =
              deps.promoteSidebarActivityToken?.(
                sidebarActivityToken,
          ...
          if (sendTargetStillDisplayed()) {
            ...
            deps.recordSendTrace(
              "sendPrompt:server-submit-existing:ui-effects-skipped",
              {
                traceId: sendTraceId,
                sessionID: existingSessionId,
                clientMessageId: sendCorrelation.clientMessageId,
          ...
          deps.holdVisibleRuntimeActivity(
            existingSessionId,
            compactCommand
              ? "sendPrompt:server-submit-existing-compact-success"
              : "sendPrompt:server-submit-existing-success",
          );
          return sessionSubmitResultFromConversationSubmit(result);
        };
      const serverSubmitExistingSessionResult =
        await submitExistingSessionWithServer();
      ...
      const shouldUseServerSubmitForFirstSession = Boolean(
        !sessionID && deps.submitConversationFromVesloWriteApi,
      );
      if (shouldUseServerSubmitForFirstSession) {
        deps.recordSendTrace(
          "sendPrompt:runtime-preflight:server-submit-first-delegated-to-create",
          {
            traceId: sendTraceId,
            targetWorkspaceId: sendTargetWorkspace?.workspaceId ?? null,
          },
        ...
        if (!conversationRunCompatibilityBridge) {
          deps.recordSendTrace(
            "sendPrompt:blocked-conversation-run-bridge-disabled",
            {
              traceId: sendTraceId,
              targetWorkspaceId: sendTargetWorkspace?.workspaceId ?? null,
              hasServerSubmit: Boolean(
                deps.submitConversationFromVesloWriteApi,
              ),
            },
          );
          ...
          return sessionSubmitBlockedResult({
            ...
            message:
              "Server-owned conversation submit is required for this send path.",
          });
        }
        if (
          !(await conversationRunCompatibilityBridge.prepare({
            cleanupPendingSidebarSession,
            sendPreflight,
            sendTargetWorkspace,
            startSendPromptBusy,
      ...
      const pendingDraftSendState = (() => {
        ...
        return {
          ...
          draftId:
            pendingDraftMeta?.id?.trim() || GLOBAL_UNPUBLISHED_PENDING_DRAFT_ID,
        };
      })();
      const consumePendingDraftAfterAcceptedSend = async (
        clearDisplayedPendingDraftState: boolean,
      ) => {
        ...
        if (pendingDraftId && deps.isTauriRuntime()) {
          try {
            const deleted =
              await deps.pendingSessionDraftsDelete(pendingDraftId);
            if (!deleted) {
              deps.markPendingDraftConsumed(pendingDraftId);
              console.warn(
                "[pendingDrafts.consume] failed to delete pending draft",
                { pendingDraftId },
              );
            } else {
      ...
      const serverSubmitMaterializationOptions =
        serverSubmitMaterializationDraft
          ? ({
              agent: sessionID
                ? (deps.agentForSession(sessionID) ?? null)
                : null,
            ...
            deps.createSessionAndOpen(initialSessionTitle, {
              ...
              onSubmitResult: (result) => {
                if (
                  ...
                  const validation = validateConversationSubmitTerminalResult(
                    result,
                    sendBoundaryValidationOptions(deps, {
                      event:
                      ...
                      context: {
                        ...
                        origin: sendCorrelation.origin,
                        targetWorkspaceId:
                          sendTargetWorkspace?.workspaceId ?? null,
                      },
                    }),
                  );
                  if (validation.ok) {
                    ...
                    serverFirstSubmitResultHolder.invalidMessage =
                      validation.message;
                  }
                }
              },
              onSubmittedRunMaterialized: (materialized) => {
                ...
                if (
                  ...
                  acceptedResult.clientMessageId !==
                    materialized.result.clientMessageId
                ) {
                  return false;
                }
                ...
                if (!workspaceId || !directory || !sessionId) {
                  deps.recordSendTrace(
                    "sendPrompt:server-submit-first-admission-missing-target",
                    {
                      traceId: sendTraceId,
                      sessionID: sessionId || null,
                ...
                deps.recordSendTrace(
                  "sendPrompt:server-submit-first-admitted-before-select",
                  {
                    traceId: sendTraceId,
                    sessionID: sessionId,
                    runId: acceptedResult.runId,
      ...
      if (serverFirstSubmitResultHolder.invalidMessage) {
        const hintedMessage = deps.addOpencodeCacheHint(
          serverFirstSubmitResultHolder.invalidMessage,
        );
      ...
      if (
        ...
        (serverFirstSubmitResult.status === "failed" ||
          serverFirstSubmitResult.status === "blocked")
      ) {
        ...
        cleanupPendingSidebarSession();
        stopSendPromptBusy();
        return sessionSubmitResultFromConversationSubmit(
          serverFirstSubmitResult,
        );
      }
      if (!sessionID) {
      ...
      if (serverFirstSubmitResult) {
        if (sidebarActivityToken) {
          deps.migrateSidebarActivityToken?.(sidebarActivityToken, {
            workspaceId:
              serverFirstSubmitResult.workspaceId?.trim() ||
              sendTargetWorkspace?.workspaceId?.trim() ||
              "",
            sessionId: sessionID,
          ...
          if (
            ...
            serverFirstSubmitResult.status === "queued"
          ) {
            sidebarActivityTokenCommitted =
              deps.promoteSidebarActivityToken?.(
                sidebarActivityToken,
        ...
        deps.recordSendTrace("sendPrompt:server-submit-first-success", {
          ...
          queueItemId:
            serverFirstSubmitResult.status === "queued"
              ? serverFirstSubmitResult.queueItemId
              : null,
          draftDisposition: serverFirstSubmitResult.draftDisposition,
        });
      ...
      if (shouldUseServerSubmitForFirstSession) {
        const message =
          "Server-owned conversation submit did not return a queued or submitted result.";
      ...
      const displayedConversationGuard = deps.captureDisplayedConversationGuard(
        materializedSessionID,
      );
      const displayedUiScopeToken = deps.activeUiScopeToken();
      const sendTargetStillDisplayed = () =>
      ...
      const compatibilityAccepted =
        await conversationRunCompatibilityBridge.submit({
          commandName,
          compactCommand,
          consumePendingDraftAfterAcceptedSend,
          draft: resolvedDraft,
      ...
      return sessionSubmitCompatibilityResultFromAccepted(

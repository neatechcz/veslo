...
test("replacement send path reverts to the original message before sending the edited draft", () => {
  assert.match(
    appSource,
    /async function sendPrompt\(\s*draft\?: ComposerDraft,\s*options: \{\s*targetSessionId\?: string \| null;\s*onMaterializedSessionId\?: \(sessionId: string\) => void;\s*pendingSession\?: PendingSidebarSessionMetadata \| null;\s*\} = \{\},\s*\): Promise<boolean> \{/,
  ...
  assert.match(
    sessionSource,
    /const promptSendOptions: \{ targetSessionId\?: string \| null; onMaterializedSessionId\?: \(sessionId: string\) => void; pendingSession\?: PendingSidebarSessionMetadata \| null \} \| undefined =[\s\S]*const accepted = await \(options\.replaceMessageId\s*\? props\.replaceUserMessageAsync\(options\.replaceMessageId, draft, targetSessionId \? \{ targetSessionId \} : undefined\)\s*: props\.sendPromptAsync\(draft, promptSendOptions\)\s*\);/s,

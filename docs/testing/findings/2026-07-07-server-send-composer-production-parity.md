...
The current server and app-boundary tests are meaningful and catch real
send-composer regressions. The causal auth-prime diagnostics gap from the
incident is now covered at source level. The next KISS step is one packaged
desktop/Pilot smoke after sidecar rebuild, not a broad new chaos suite.

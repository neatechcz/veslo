...
- Move to user skills performs the same user-skill copy, then deactivates the original workspace-local source. If deactivation fails, the operation reports the failure instead of silently hiding it.
...
- Deactivate removes the exact writable active-workspace installation from the active runtime. User-skill, server-controlled, read-only, and non-active-workspace deactivation paths stay hidden until those mutation paths are explicitly supported.
...
- Restore version restores a deleted registry installation or re-selects an available version through the registry proxy, depending on the selected location context. The UI must refresh version and installation metadata before presenting the updated state.

...
Managed Codex inference follows the model in the user's AI access policy, not a model setting on the OAuth credential. The GPT-5.6 Sol rollout keeps `gpt-5.6-sol` as the sole allowed model on enabled and disabled Codex policies. If the assigned credential does not support the model, cannot authenticate, is exhausted, or otherwise fails, the request fails explicitly and the policy remains on GPT-5.6 Sol for operator reconnect or recovery; the runtime does not fall back to GPT-5.5.

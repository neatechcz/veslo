...
Managed Codex inference follows the model in the user's AI access policy, not a model setting on the OAuth credential. The GPT-5.6 Sol rollout keeps `gpt-5.6-sol` as the sole allowed model on enabled and disabled Codex policies. If the assigned credential cannot authenticate, is exhausted, or otherwise becomes ineligible, lazy repair can replace it with another healthy eligible Codex credential while retaining the GPT-5.6 Sol policy. The request fails explicitly only if repair cannot find a replacement that can serve it; an unsupported-model response from the selected credential remains a failure when no such replacement is available. The runtime does not fall back to GPT-5.5.

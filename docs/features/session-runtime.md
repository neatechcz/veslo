...
An ordinary managed-AI send freshly checks its server-owned OpenCode config before admission. A transient transport failure while reading that config from the loopback Veslo server does not reject the send when the current managed-config intent exactly matches an in-memory intent that was already verified during this app process. The identity includes the workspace and server-workspace target, managed profile and model roster, local server URL and token, provider routing, DEN authorization revision, and runtime authorization inputs. This degradation is unavailable when a config reload is pending, any identity component changed, no prior verification exists, or the server returned an API/auth error. Those cases remain fail-closed. The degraded decision is recorded as `managed-ai-config-sync:degraded-last-verified`; the underlying timeout remains observable, and the next send performs a fresh read again.

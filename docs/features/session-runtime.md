...
New DEN sign-ups automatically receive a healthy eligible Codex credential with GPT-5.6 Sol as the default model, using canonical id `gpt-5.6-sol`, when one is available. The moving `gpt-5.6` alias is not used. The assignment is applied during the auth sign-up hook, marked `auto_assigned`, and skipped without blocking account creation when no eligible credential exists.

Standalone AI Gateway admin exposes model choices for assignments through the credential model-list endpoint. OpenAI-compatible credentials use live upstream `/models` discovery. Codex OAuth credentials use the gateway-owned Codex model catalog, with `gpt-5.6-sol` as the default and first entry, so admins can select from known Codex model ids without relying on experimental Codex model-discovery internals. Older catalog entries remain visible for explicit administration; they are not automatic fallback models.

Managed Codex inference follows the model in the user's AI access policy, not a model setting on the OAuth credential. The GPT-5.6 Sol rollout keeps `gpt-5.6-sol` as the sole allowed model on enabled and disabled Codex policies. If the assigned credential cannot authenticate, is exhausted, or otherwise becomes permanently ineligible, lazy repair can replace it with another healthy eligible Codex credential while retaining the GPT-5.6 Sol policy; the request fails only when no eligible replacement exists. Target-model unsupported intentionally leaves the selected credential eligible, so that status does not trigger lazy repair. Its GPT-5.6 Sol probe or request can fail even when another credential exists, until an admin explicitly assigns a credential that supports the target model or explicitly selects a supported credential/model outside the forced rollout. The runtime does not fall back automatically to GPT-5.5.

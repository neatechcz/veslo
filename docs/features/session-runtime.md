...
Every inference provider request resolves in the fixed order authenticated session → DEN billing entitlement → user enablement/provider assignment → single global active model → compatible credential. DEN exposes only the minimal organization entitlement decision; AI Gateway caches that decision briefly and fails closed when DEN is unavailable. Billing denial or lookup failure stops before access initialization, model, or credential resolution. A user-supplied model cannot switch the runtime model; a conflicting model, provider, or credential override is rejected.

DEN signup establishes verified identity and active organization membership but does not select Managed AI routing. The automatic-access service has entry-point-specific guards: inference reaches it only after authentication and positive DEN entitlement, where a missing access record is created lazily as enabled; self-access can initialize that record after authentication without a billing entitlement check; an organization-qualified admin GET can do so after admin authentication, active target-membership validation, and scoped organization authorization without a billing entitlement check. An explicit disabled record is returned unchanged on every path. Signup does not create a per-user model choice or default model.

DEN provider inference remains retired through the code-level dependency `denInferenceMode: "retired"` and returns `410 den_managed_ai_inference_retired`. Historical per-user inference exists only when code explicitly constructs the DEN proxy with `denInferenceMode: "legacy_rollback"`; there is currently no operator environment switch for that mode. Compatibility reads do not mutate assignments or recover model authority from retained columns. Direct embedded DEN enablement requires an injected automatic resolver and fails closed when none is configured.

Standalone AI Gateway platform administration configures the global enabled-backend-model catalog and its single active model. That single global active model is the only runtime model authority. Credential-specific model listing is infrastructure evidence for validating that policy and populating its catalog, not a model choice attached to a user assignment. OpenAI-compatible credentials use live upstream `/models` discovery, while Codex OAuth credentials use the gateway-owned Codex model catalog without relying on experimental Codex model-discovery internals. `/api/me/ai-access`, the exact-user compatibility aliases, inference authorization, and organization-qualified admin access all share the same automatic-access service behind their distinct guards. Enabled reads compose the current active model as `effectiveModel`, constrain `allowedModels` to that model, and keep `selectableModels` empty.

For direct cloud compatibility, `/api/me/ai-access` and `/api/users/:userId/ai-access` use the same authentication-only self-access boundary; `/ai-gateway/users/:userId/ai-access` remains the equivalent mounted alias. The user-id routes return the same response as `/me` only when the path user id exactly matches the bearer-token user. A mismatch fails before access or model-policy lookup. The desktop and local Veslo server continue to use `/me`; the explicit route neither restores trusted identity headers nor performs the inference billing check.

The desktop displays `effectiveModel` as read-only managed infrastructure state.
Self-access exposes no alternative selectable models, so a local Session model
selector preference cannot present a Managed-AI choice or change the model used
for a send. Scheduled automations still send only `model: null` to erase legacy
target state. Legacy global, session, automation, proof-cache, transcript, and
request model choices are ignored or rejected and cannot override the global
active model. Missing, malformed, or provider-mismatched `effectiveModel` data
fails closed instead of reviving a legacy choice.

The standalone admin shell keeps platform administration and organization workspaces as separate route areas. Platform overview, organization directory, AI infrastructure, platform users, and global audit routes never retain an organization id. Organization overview, members, domains and invites, billing, AI access, and audit routes always include the authorized organization id in `/admin/organizations/:orgId/...`; the persistent organization selector appears only in that workspace and preserves the current organization subpage when switching. Global user creation, profile edits, platform-admin assignment, disable, and delete actions are available only on the canonical Platform Users route, which cannot mutate organization-scoped AI access. The organization AI Access member modal exposes only the AI Access switch and, to platform admins, the platform-admin permission switch; provider, credential, model, redemption, and routing inputs are absent. Its organization-qualified write accepts only `enabled`, while AI Infrastructure retains global model and credential controls. Pending organization-scoped reads and mutations cannot update status, close dialogs, redirect, or render into a different organization after a route switch. Organization Billing proxies Den's canonical billing API without duplicating Stripe logic, preserves Den validation responses, and exposes manual controls only to platform admins.

Organization Audit merges the authoritative DEN organization stream with organization-scoped Gateway events. Returned rows carry `den` or `ai-gateway` source labels and source-qualified stable ids, are ordered newest-first, and are hard-limited only after merging. The facade fails closed rather than showing a partial stream when either source is unavailable. DEN-owned mutations are not re-audited in Gateway. Gateway-owned AI-access updates require the canonical organization route and a routed organization that matches the target user's DEN membership. Required credential response data is prepared before the transaction, then policy and audit are committed together with the authenticated actor and validated organization scope. Legacy Gateway rows without organization scope remain visible only in global audit.
...
In desktop local workspaces, the app reads managed-AI access policy from standalone AI Gateway, but generated OpenCode provider config must still route through the active local Veslo server. Remote DEN/Veslo URLs are not valid provider `baseURL` targets for local-first desktop execution. The generated gateway model headers are run-scoped, not workspace-scoped: config keeps `x-veslo-session-id = ${OPENCODE_SESSION_ID}`, and the local proxy resolves that placeholder from OpenCode's real `x-session-id` request header. Legacy `x-veslo-workspace-id` model headers are stale runtime state and must be scrubbed from managed provider config.
...
OpenAI-compatible custom providers are admin-managed. The desktop app receives provider `openai_compatible` and a model id in the read-only AI access policy, writes local OpenCode routing for `@ai-sdk/openai-compatible`, and sends prompts through the local Veslo server route `/ai-gateway/providers/openai_compatible/v1/chat/completions`. Standalone AI Gateway resolves the assigned platform credential, reads the encrypted base URL and API key, forwards to `${baseUrl}/chat/completions`, and records usage against provider `openai_compatible`.
...
User managed-AI access records expose only enablement as an administrator-controlled field; provider, credential assignment, and assignment origin remain server-owned technical state. Historical per-user `provider`, `default_model`, and `allowed_models_json` values are not runtime authority. Enabled Gateway reads and inference resolve the provider and model from the current single global active model; self-access returns no selectable alternatives. Admin writes accept only `enabled`; legacy model fields and every provider, credential, redemption, or routing field are rejected. Enabling resolves the active model provider and compatible capability evidence before the write. Platform model-policy writes and AI-access assignment writes serialize on the singleton model-policy row before mutating. Readiness requires an enabled access record, a configured current global model policy, successful configured provider probes, and a healthy credential with capability evidence for the active model. It does not read historical per-user provider, default-model, or allowed-model routing fields, so historical provider mismatches are not readiness failures; transient capability lookup failures still fail closed as `no_healthy_credential_for_active_model`.

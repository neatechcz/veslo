...
DEN signup establishes verified identity and active organization membership but does not select Managed AI routing. After AI Gateway authenticates the user and confirms DEN entitlement, a missing access record is created lazily as enabled from the current global model policy and compatible credential capability evidence. An explicit disabled record is returned unchanged. Signup does not create a per-user model choice or default model.
...
User managed-AI access records expose only enablement as an administrator-controlled field; provider, credential assignment, and assignment origin remain server-owned technical state. The gateway composes the read-only `effectiveModel` from the current platform model policy, so historical per-user model columns cannot influence routing or API responses. Admin writes accept only `enabled`; legacy model fields and every provider, credential, redemption, or routing field are rejected. Enabling resolves the active model provider and compatible capability evidence before the write. Platform model-policy writes and AI-access assignment writes serialize on the singleton model-policy row before mutating. Readiness reports enabled assignment/provider mismatches as `ai_access_policy_provider_mismatch` instead of declaring the gateway ready, while transient capability lookup failure fails closed as a service error.

...
User managed-AI access records expose only enablement as an administrator-controlled field; provider, credential assignment, and assignment origin remain server-owned technical state. Historical per-user `provider`, `default_model`, and `allowed_models_json` values are not runtime authority. Enabled Gateway reads and inference resolve the provider and model from the current single global active model; self-access returns no selectable alternatives. Admin writes accept only `enabled`; legacy model fields and every provider, credential, redemption, or routing field are rejected. Enabling resolves the active model provider and compatible capability evidence before the write. Platform model-policy writes and AI-access assignment writes serialize on the singleton model-policy row before mutating. Readiness reports enabled assignment/provider mismatches as `ai_access_policy_provider_mismatch` instead of declaring the gateway ready, while transient capability lookup failure fails closed as a service error.

...
Archive and restore mutations are ordered per archive owner, and legacy local archive state is merged into existing server state without discarding entries that cannot yet be resolved from the loaded session list.

...
2. Deploy DEN before AI Gateway so the minimal authenticated `GET /v1/managed-ai/entitlement` facade, verified organization provisioning, and safe organization errors are available first; verify those boundaries.
3. Deploy AI Gateway model-policy/admin support, then configure and verify the global model policy with at least one enabled backend model and exactly one active model backed by compatible credential evidence. That active model is the exclusive runtime authority. The Gateway automatic-access service uses the policy behind different entry-point guards: authenticated self-access has no billing check, inference requires positive DEN entitlement, and organization-qualified admin reads require scoped membership and authorization. DEN signup does not need a provider-policy resolver.

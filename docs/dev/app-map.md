...
- Standalone AI Gateway owns user AI-access records, the global model policy, platform credentials, runtime model enforcement, credential rotation, usage, and the canonical `/admin` UI. Its automatic-access service derives enabled technical routing from the single global active model and capability evidence while preserving explicit disablement. Self-access reaches it after authentication without a billing check; inference reaches it only after positive DEN entitlement; organization-qualified admin reads reach it only after admin authentication, scoped membership validation, and organization authorization.

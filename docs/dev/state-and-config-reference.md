...
read-only `effectiveModel` from the single global active model. Self-access
keeps `selectableModels` empty, so neither these legacy keys nor a user request
can select another managed model.
...
Managed-AI technical routing may use `openai`, `anthropic`, `codex_oauth`, or `openai_compatible`, but the only administrator-controlled per-user field is enablement. Provider, credential assignment, and `assignment_origin` are server-derived state. The global AI Gateway platform model policy may retain multiple enabled backend models as infrastructure catalog entries, but its exactly one global active model is the exclusive runtime authority. The desktop reads that model as `effectiveModel`; self-access keeps `selectableModels` empty and exposes no alternatives. A user request cannot switch the model, provider, or credential. `assignment_origin` is `auto_assigned` for lazy Gateway defaults and `admin_assigned` for explicit enablement edits or legacy rows.

Inference reaches lazy initialization only after authentication and positive DEN organization entitlement, at which point a missing AI-access row becomes enabled. Authenticated self-access reaches the same initializer without a billing entitlement check. An organization-qualified admin GET reaches it after admin authentication, active target-membership validation, and organization authorization, also without a billing entitlement check. An explicit disabled row is preserved on every path. Provider, model, and credential routing are derived from the global policy and current capability evidence. Organization-qualified admin writes toggle only `enabled`, and technical routing fields are rejected. Historical per-user `provider`, `default_model`, and `allowed_models_json` columns are not runtime authority in standalone AI Gateway. DEN provider inference defaults to `denInferenceMode: "retired"` and returns `den_managed_ai_inference_retired`; those columns can be consulted only if code explicitly constructs the DEN proxy dependency with `denInferenceMode: "legacy_rollback"`, for which there is currently no operator environment switch. Compatibility reads are mutation-free, and direct embedded DEN enablement fails closed without an injected automatic resolver.

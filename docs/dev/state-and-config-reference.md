...
Windows installer/onboarding prerequisite repair must install or update the WSL package without creating a personal default distro, then import the managed `VesloSandbox` distro. Real WSL/VesloSandbox setup failures must be logged as failed runtime setup instead of reported as prepared runtime. MSI package installs may still complete to avoid a generic Windows Installer custom-action error; first-run onboarding and Settings repair must then continue the same setup path, and lazy first-prompt startup must block with a repairable prerequisite error. If Windows feature enablement requires a restart, MSI setup must leave a restart-required marker, offer the user an installer-time restart prompt after file installation has finalized, disable installer-time app launch, and register a RunOnce continuation. After reboot, the continuation must retry prerequisite installation before sandbox import; that restart-required continuation is the one allowed pending state.

Installer runtime setup must run under the target Windows user, not `SYSTEM`, because WSL distros and `RunOnce` continuations are per-user. A `SYSTEM` invocation must log a failed runtime setup instead of reporting a prepared runtime.

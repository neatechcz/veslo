...
For a manual dev-runtime reproduction, enable the existing runtime performance audit in DevTools and reload before the action under investigation:

```js
localStorage.setItem("veslo:runtime-perf-audit", "1");
location.reload();
```

The app records only semantic connection changes under `workspace.requests`, so status polling does not flood the trace. Filter `window.__vesloPerfLogs` for `veslo-connection-state` to see the previous and current server reachability, runtime readiness, runtime-chain diagnostic fields, and a bounded probe failure reason. `veslo-local-server-ensure` records the outcome of a local runtime readiness attempt, including whether it restarted the owned server. During Tauri development the same entries are emitted as `[OWPERF]` records and forwarded through the native runtime-log path. Remove the local-storage key and reload to stop audit-only records.
...
The current managed Codex catalog and DEN sign-up default is GPT-5.6 Sol with canonical model id `gpt-5.6-sol`. Veslo does not use the moving `gpt-5.6` alias and does not automatically fall back to GPT-5.5.

Codex authentication state and model policy are separate durable data. `credential_record`, `credential_secret`, and `credential_binding` own the credential id, encrypted OAuth auth JSON, provider binding, and health lifecycle. `user_ai_access_policy.default_model` and `user_ai_access_policy.allowed_models_json` own the assigned model; `credential_id` and `assignment_origin` remain part of the policy assignment. The GPT-5.6 Sol migration selects every `provider='codex_oauth'` policy, including disabled rows, and transactionally changes `default_model`, `allowed_models_json`, and `updated_at` on rows whose model policy changes. It preserves `enabled`, credential records, encrypted secrets, bindings, stored health, policy credential ids, and assignment origins.

The rollout probe is intentionally broader than normal eligible-credential selection. It loads all non-deleted `codex_oauth` records, including unhealthy ones, sorts them by credential id, and probes them sequentially through one shared model-configured status provider. Each failure is classified and retained while later credentials continue. Refreshed Codex auth is written back through `credential_secret` encryption; output is limited to the target model, safe credential metadata, and outcomes. Authentication failure, exhaustion, and permanent ineligibility can cause lazy repair to bind another healthy eligible credential while preserving `gpt-5.6-sol`; the request fails only if no eligible replacement exists. By contrast, `unsupported_model` intentionally leaves the credential eligible and therefore does not trigger lazy repair. Its probe or request can fail even when another credential exists, until an admin explicitly reassigns/selects a supported credential or model. Neither outcome changes the policy or triggers an automatic GPT-5.5 fallback.

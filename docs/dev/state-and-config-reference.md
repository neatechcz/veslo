...
The same native diagnostics owner also maintains a local, redacted retrospective ring for the last 10 minutes of desktop sidecar and UI diagnostics. It is segmented on disk, has a strict 50 MiB total budget, and keeps only segment metadata in memory. It never uses the normal continuous debug-log forwarding path. The feedback modal can explicitly attach this snapshot; the native owner writes it into the existing capture queue and keeps it local until the feedback report itself succeeds. Only then is the attachment queued for bounded direct-to-Den batches, and Den records the `diagnosticCaptureId` on the feedback report for support lookup. After a feedback report succeeds, the modal remains visibly in its attachment-delivery state and cannot be dismissed until the native owner confirms delivery or reports a terminal failure; it explicitly tells the user not to close Veslo while delivery remains pending. This is independent of GlitchTip exception monitoring.

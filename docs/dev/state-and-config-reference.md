...
The initial archive list waits until the active Veslo server has completed an authenticated health check. Cold-start connection failures therefore remain part of server readiness instead of surfacing as an archive error; a load attempted after readiness still reports failures and retries after a later health check.
...
- For an archive-list response-contract failure, monitoring may attach a sanitized request origin and pathname, fixed response kind, HTTP status, media-type category, actual canonical content type, and a bounded/redacted preview. The pathname excludes query/fragment data; the preview redacts credentials, bearer values, sensitive assignments, URL query values, and home paths. It must not attach headers, a full raw URL, credentials, or an unbounded raw response body.
...
The frontend Veslo server connection status represents the HTTP server/auth/capability path. A successful `/health` plus authenticated `/capabilities` probe is a connected Veslo server even when the active local workspace runtime chain is still lazy, detached, or unavailable. Runtime-chain readiness is recorded as diagnostic/action-specific state and may still explain why a workspace action is unavailable, but it must not downgrade the visible Veslo server status or make capability-backed Skills and registry controls report that the server itself is disconnected.
...
- Direct Den fallback HTTP status logs include a short sanitized response-body excerpt. HTTP 400 is treated as an invalid direct-fallback payload for that flushing file: direct-eligible events are dropped from that file instead of being retried indefinitely, while non-direct events remain in the local spool when present.
...
Production, staging, prerelease, and debug Tauri runtimes expose a user-started **Send a diagnostic capture** control in Settings. It creates a native-owned, redacted capture for at most 120 seconds and 2 MiB of fully serialized log events; the active capture can also be stopped early with the **Stop capture** action. Direct upload batches contain up to 1,000 events or 512 KiB of the serialized request body, whichever limit is reached first; the 2 MiB total capture budget remains unchanged. Capture events use a UUID `captureId`, are queued separately from the normal desktop spool, bypass local `/debug-logs`, and use a stable direct-to-Den idempotency key. The desktop starts its orchestrator sidecar with structured JSON logging; capture retains `info`, `warn`, or `error` only from a valid structured logger record. Server-owned send-flow records use the same envelope in JSON-log mode, including typed conversation-submit failures. Arbitrary stdout/stderr remains nullable-level and is never classified from its stream or text. Queue reads, writes and rewrites are serialized; corrupt or missing capture queues become terminal `undeliverable` states rather than being silently discarded. The journal binds the queue to the user/org that started it and reports `uploaded` only after all queued capture events (including the final summary) have been accepted. A fully delivered capture is retained only as an in-process confirmation for the current Settings view; its queue and journal are deleted before the next desktop start, and legacy completed journals are pruned on startup. Client failures use persisted backoff, while a server validation rejection is terminal. If the account changes, the process restarts, the byte cap is hit, or the 24-hour queue retention expires, the terminal journal state records the reason instead of retrying under a different identity. Release builds are compile-time gated only by the explicit `VESLO_USER_DIAGNOSTIC_CAPTURE=1` opt-in. Every capture still requires a signed-in user and is uploadable only to the owned production or staging Veslo API endpoint.

The same native diagnostics owner also maintains a local, redacted retrospective ring for the last 10 minutes of desktop sidecar and UI diagnostics. It is segmented on disk, has a strict 50 MiB total budget, and keeps only segment metadata in memory. It never uses the normal continuous debug-log forwarding path. The feedback modal can explicitly attach this snapshot; the native owner creates, queues, and discards its filesystem snapshot on a blocking worker rather than the Tauri command runtime, then writes it into the existing capture queue and keeps it local until the feedback report is durably stored in the owned Den MySQL database. Only then is the attachment queued for bounded direct-to-Den batches, and Den records the `diagnosticCaptureId` on the feedback report for support lookup. Desktop server and orchestrator sidecars both use structured JSON logs, so safe watcher attributes such as a skill-reload reason code, retry budget, and operation ID remain available in a feedback capture. Feedback persistence does not call YouTrack or another third-party ticketing service. If feedback persistence itself fails, the unqueued snapshot is discarded so it cannot block a subsequent feedback attachment. After a feedback report succeeds, the modal remains visibly in its attachment-delivery state and cannot be dismissed until the native owner confirms delivery or reports a terminal failure; it explicitly tells the user not to close Veslo while delivery remains pending. This is independent of GlitchTip exception monitoring.
...
AI Gateway `/health` is process liveness only. Use `/readiness` when the product, admin UI, or monitors need to show AI inference availability: it checks the configured global active model, compatible credential evidence, upstream provider reachability, and enabled AI access. The local server exposes the same frontend-visible readiness through `/ai-gateway/readiness`; send-time provider proxy failures remain authoritative and continue returning normalized upstream failure diagnostics.

Provider requests use the fixed authorization order session → DEN billing entitlement → user enablement/provider assignment → global active model → credential/lease/token brokerage. AI Gateway calls DEN `GET /v1/managed-ai/entitlement`, caches allow and deny decisions briefly, and never caches lookup failures. Billing denial returns `managed_ai_entitlement_denied`; DEN/network/malformed-response failure returns `managed_ai_entitlement_unavailable`. Both fail before model or credential work.
...
Desktop local workspaces separate the managed-AI access-policy source from the OpenCode provider routing target. The app may load the user's policy and gateway token from standalone AI Gateway, but generated OpenCode config must not persist live gateway credentials in `opencode.json` or `opencode.jsonc`. The generated provider `baseURL` points at the active local Veslo server, and local server authentication is referenced with `{env:VESLO_OPENCODE_SERVER_CLIENT_TOKEN}` so prompts keep flowing through the local-first runtime without commit-prone secrets. The generated model headers include only `x-veslo-session-id = ${OPENCODE_SESSION_ID}`; workspace identity is request/runtime state and must not be persisted into provider model config. The local proxy resolves placeholder session ids from OpenCode's real request session header (`x-session-id`) and falls back to workspace correlation only for legacy single-workspace active-run cases. Legacy `x-veslo-workspace-id` model headers are treated as stale config and are scrubbed by managed-AI config sync. The cloud gateway bearer and authenticated organization id are held only in local Veslo server runtime memory and are never persisted to OpenCode config. Runtime authorization is stored by local actor plus authenticated organization. A workspace-scoped access prime binds that organization to the server-validated workspace, and server-owned run admission snapshots the binding into immutable active-run context. The lifecycle store persists a separate, non-secret, stateful recovery descriptor with its issue/expiry time: exact run, conversation, directory, OpenCode session, current attached engine-owner tuple, actor-token hash, and organization scope. It is atomically updated and terminalized with its run, migrated with its workspace, and exposed only to the authenticated lifecycle recovery endpoint—not normal lifecycle responses. After a local server worker replacement, the server restores context only when one active unexpired descriptor and a fresh lifecycle read prove the same attached owner tuple. It then gates provider forwarding for a bounded interval until a desktop access prime proves the same workspace, actor hash, and organization. It never restores or derives a bearer token from the engine, a model header, or durable storage. Expired evidence ends the exact run through normal lifecycle handoff with `gateway_runtime_authorization_recovery_timeout`; absent, ambiguous, or stale-owner evidence is terminalized with `gateway_runtime_authorization_recovery_identity_mismatch`. Neither path may degrade into a generic gateway 401. The health endpoint `instanceId` is the server worker generation: the desktop invalidates its short in-process prime cache and re-primes whenever that generation changes, including a restart too brief for the connection state to visibly become disconnected. Provider forwarding resolves only that session/run binding; caller-supplied provider organization and workspace headers are stripped and cannot select another organization. Missing or ambiguous multi-organization context fails closed, while a legacy unscoped actor remains compatible only when it has exactly one authorization.

A short-lived in-memory terminal outcome retains only the exact non-secret session correlation after recovery terminalization. It lets a retried matching provider request receive the same safe recovery result instead of falling through to a generic authorization error; it cannot authorize, revive, or replay the run.
...
After local admission, the server normally allows up to 30 seconds for an AI-gateway provider request to start (`VESLO_AI_GATEWAY_PROVIDER_START_TIMEOUT_MS`). If it does not, the server aborts the exact OpenCode session before marking the run failed and releasing its conversation queue; this prevents later queued prompts from being admitted behind the stuck assistant message. If that abort cannot be completed, the reservation persists the exact session target and retries at a 5-second cadence after checking whether the lifecycle has naturally become terminal. A missing or still-active lifecycle state never releases the queue in this recovery path, including after a local-server restart. This is a terminal fallback for a missing provider start, not a timeout on a provider stream that has already begun.

Orchestrator proxy upstream socket closes are classified before marking the shared OpenCode engine unhealthy. Transient `GET /event` SSE closes are non-fatal. Non-event socket closes remain fatal during normal runtime, but transient closes observed after orchestrator shutdown has started are marked non-fatal and carry `shutdown: true` plus `nonFatalEngineError: true` in runtime traces.
...
Legacy DEN managed-AI persistence uses `MANAGED_AI_DATABASE_URL`. Standalone AI Gateway uses `AI_GATEWAY_DATABASE_URL` and is the normal-runtime authority for assignment, credential, eligibility, model-policy, and usage data. Sharing a database does not grant DEN co-ownership of those runtime decisions; DEN's current production role remains identity, organization membership, and billing entitlement.
...
For `codex_oauth`, OpenCode still owns local tool execution and calls the local Veslo server route `/ai-gateway/providers/codex_oauth/v1`; standalone AI Gateway keeps the Codex OAuth auth JSON server-side, translates the OpenAI-compatible request to the ChatGPT Codex Responses endpoint, and translates the response back for OpenCode. Live upstream discovery for `openai_compatible` credentials and the gateway-owned Codex catalog are infrastructure evidence for platform-policy validation and catalog input, not user-assignment model pickers. Codex status probes run from temporary Codex homes with the gateway's probe model, and refreshed probe auth JSON is written back to the credential's encrypted secret when Codex rotates it. If a probe reports refresh-token reuse, the standalone admin service quarantines the credential as unhealthy; the admin Reconnect action replaces the stored auth JSON in place and marks the same credential healthy again. If a probe reports that a specific model is unsupported for a credential's ChatGPT account, the credential remains available while that model is excluded from capability evidence for that credential. Any assigned Codex row can be repaired to another healthy eligible Codex credential when its credential becomes unhealthy, revoked, missing, permanently unavailable, or exhausted; repair preserves the original assignment origin and uses only the platform active model for compatibility. For `openai_compatible`, OpenCode uses the local Veslo server route `/ai-gateway/providers/openai_compatible/v1`; the upstream custom base URL and API key remain server-side in standalone AI Gateway's encrypted secret store.
...
Registry availability is exposed through authenticated `/capabilities` as
`skillRegistry: { configured: boolean }`. `configured` is true when the local
server resolves a registry base URL from `VESLO_SKILL_REGISTRY_BASE_URL`,
`skillRegistryBaseUrl` in server config, or the Den API base fallback
(`denApiBase`/`VESLO_DEN_API_BASE`). This flag describes configured registry
reachability, not whether any specific workspace runtime chain is warm.
...
The Skills page now treats installed skills as an app-wide inventory with filterable location rows. UI filters are local presentation state. Registry-backed install preparation can create the initial skill, version, and installation through the local server proxy. The bulk Publish action for review requests is available only for exactly one selected active, readable user-global or workspace skill, with a connected Veslo server, `skills.write`, and `/capabilities.skillRegistry.configured`; disabled states use specific `skills.bulk_publish_*` localized tooltips. Review dialog request submit creates the skill, version, and review request in sequence before closing on success. Registry-backed publish, approval, remove, restore, and workspace skill-set controls should call the local proxy only when the selected row has concrete registry identifiers for the action target, then refresh registry metadata and local inventory after success.

...
The operator Codex rollout probe is intentionally broader than normal eligible-credential selection. It loads every non-deleted `codex_oauth` record, including unhealthy records, sorts them by credential id, and probes them sequentially through one model-configured status provider. Each classified failure is retained while later credentials continue. Refreshed Codex auth is persisted through the encrypted secret store, and output is limited to the target model, safe credential metadata, and outcomes. The probe does not change the global platform model policy or user assignments.

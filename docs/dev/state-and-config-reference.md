...
The rollout probe is intentionally broader than normal eligible-credential selection. It loads all non-deleted `codex_oauth` records, including unhealthy ones, sorts them by credential id, and probes them sequentially through one shared model-configured status provider. Each failure is classified and retained while later credentials continue. Refreshed Codex auth is written back through `credential_secret` encryption; output is limited to the target model, safe credential metadata, and outcomes. A probe failure does not change the `gpt-5.6-sol` model policy or trigger a GPT-5.5 fallback. At request time, lazy repair may bind another healthy eligible credential while keeping that model; inference fails only when no eligible replacement can serve the request.

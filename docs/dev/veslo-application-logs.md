...
For clean-install WSL runtime setup issues, also collect `%ProgramData%\Veslo\logs\wsl2-prerequisite-installer.log`, `%LOCALAPPDATA%\Veslo\logs\wsl2-client-installer.log`, and `%LOCALAPPDATA%\Veslo\logs\wsl2-sandbox-installer.log` when those files exist. MSI clean installs can complete while logging `VESLO_RUNTIME_SETUP_RESULT=failed` so first-run onboarding can show the repair path instead of a generic Windows Installer custom-action error. When setup logs `VESLO_RUNTIME_SETUP_RESULT=restart_required`, the MSI should also write `%LOCALAPPDATA%\Veslo\runtime-setup-restart-required.marker`, run a post-install restart prompt, log the `shutdown.exe` scheduling result, suppress installer-time app launch, and rely on the registered RunOnce continuation to keep preparing WSL after the user signs back in. NSIS clean installs also print these paths in the installer detail log and stop on non-restart runtime setup failures.
...
If elevated `wsl2-prerequisite-installer.ps1 -Install` returns `1` or `2`, inspect the same prerequisite log for `Script revision: optional-feature-first-20260623`. Current helpers retry the modern `wsl --install --no-distribution --web-download` path with the older no-distribution syntax, then enable `Microsoft-Windows-Subsystem-Linux` and `VirtualMachinePlatform` through PowerShell `Enable-WindowsOptionalFeature` before falling back to DISM. The client installer and Settings/onboarding repair output should include a `Latest WSL prerequisite helper transcript` section after elevated repair attempts.

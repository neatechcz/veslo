...
- If a normal Codex status probe reports that a specific model is unsupported for the credential's ChatGPT account, the credential remains visible and that model is removed from the credential's admin model choices. The forced GPT-5.6 Sol rollout reports unsupported, authentication-failing, exhausted, and otherwise failing credentials for reconnect or recovery without falling back to GPT-5.5 or silently changing the migrated model policy. On a runtime request, lazy repair can replace the assigned credential with another healthy eligible Codex credential while retaining `gpt-5.6-sol`. Inference fails only if repair cannot find a replacement that can serve the request; an unsupported-model response from the selected credential remains an explicit failure when no such replacement is available.

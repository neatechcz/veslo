...
- Codex OAuth credentials are refreshed from the selected credential detail. Admins can rename the credential, prepare a short-lived local upload command, run `node scripts/admin/codex-auth-upload.mjs` from a Veslo checkout, complete Codex device login locally, and upload the resulting `auth.json` through the one-time URL. The helper stores local Codex login material under `~/.veslo/codex-auth/<credential>` by default, validates that `id_token`, `access_token`, `refresh_token`, and `account_id` are present, and uploads the full JSON to the selected admin service. For existing credentials, the service replaces the encrypted secret, runs a fresh server-side Codex status probe that can persist any rotated auth JSON, and only then marks the existing credential healthy while preserving credential id, usage, audit, alert, and assignment history. If the validation probe reports reused refresh-token auth or another unavailable status, the upload session remains usable for retry and the credential is not reconnected.
...
- Codex OAuth credentials should use a dedicated server/runtime ChatGPT account. Do not reuse the same login material in another long-running runtime. If the Codex status probe reports that a refresh token was already used, the admin service marks that credential unhealthy so it is hidden from new assignments and eligible users can fail over to another healthy Codex credential. Upload/reconnect validation suppresses competing admin status probes for the same credential, and refresh-token-reuse quarantine writes are ignored when the credential is already unhealthy or has been reconnected since the stale probe started.

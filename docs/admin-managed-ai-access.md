...
- Admin AI-access reads and writes use only the organization-qualified `GET` and `PUT` routes under `/admin/api/organizations/:organizationId/members/:userId/ai-access`. They require the canonical Organization AI Access route, managed-AI admin capability, access to the routed organization, and exactly one active target-user membership returned by the scoped organization-member API. Missing, inactive, duplicate, or malformed membership evidence fails closed. The former unqualified `/admin/api/users/:userId/ai-access` routes are removed.

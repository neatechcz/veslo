...
- Optionally configure the release-only GlitchTip source-map values `VESLO_GLITCHTIP_URL`, `VESLO_GLITCHTIP_ORG`, and `VESLO_GLITCHTIP_PROJECT` as GitHub Actions variables plus `VESLO_GLITCHTIP_AUTH_TOKEN` as a secret. The source-map pipeline and its CLI are enabled only when all four values are present. Then publish jobs build the frontend once, inject debug IDs, and upload that build's maps under `veslo@<version>`; missing credentials or a failed CLI/upload only warn and do not block packaging. Source maps and their references are removed before Tauri packages the frontend. The token must never be exposed to the packaged application, release evidence, or a user setting.

...
Automatic trials are organization-scoped and last 14 days. DEN grants only when the organization has at least one registered domain and every current domain is unclaimed, then records immutable claims for all of them. Existing manual or automatic trials backfill claims without changing their expiry; later domains are consumed by that same trial. Claims survive domain deletion and reassignment, so a consumed domain cannot unlock another trial, and membership changes never reset or extend it. Active members inherit the organization entitlement. After verified active membership exists, standalone AI Gateway can lazily create missing user access as enabled: self-access requires authentication but no billing check, inference additionally requires positive DEN entitlement, and organization-qualified admin reads require scoped membership and authorization. Explicit disablement is preserved. The single global active model is the exclusive runtime model authority; end users and DEN cannot select or switch a Managed AI model or provider.

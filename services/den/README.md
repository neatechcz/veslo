...
- `RESEND_API_KEY` optional Resend API key used to send Better Auth verification and password reset emails. Blank or unset values disable email verification and password reset delivery.
- `AUTH_EMAIL_FROM` optional sender address for auth emails, for example `Veslo <auth@veslo.neatech.com>`. Blank or unset values disable email verification and password reset delivery.
- `PORT` server port

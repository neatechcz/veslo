...
export function createAnthropicProxyRouter(
  ...
  async function executeLeaseRequest(
    ...
    const upstreamResponse = await deps.anthropicTransport.messages({
      upstreamAuth,
      body,
    })
...
function getAnthropicRequestId(upstreamResponse: ProviderTransportResponse) {
  return (
    upstreamResponse.headers?.["x-upstream-request-id"] ??
    upstreamResponse.headers?.["request-id"] ??
    upstreamResponse.headers?.["x-request-id"] ??
    getString(getRecord(upstreamResponse.body), "id") ??
    `anthropic_req_${randomUUID()}`
  )
}

...
test("POST /admin/api/credentials rejects openai-compatible credentials until baseUrl storage is implemented", async () => {
  const session = createSession()
  const calls = {
    secrets: [] as Array<{ kind: string; apiKey?: string; authJson?: string }>,
    credentials: [] as Array<{
  ...
  try {
    ...
    assert.deepEqual(await response.json(), { error: "invalid_provider" })
...
test("POST /admin/api/credentials creates a shared codex_oauth credential", async () => {
  ...
  try {
    ...
    assert.equal(response.status, 200)
    assert.deepEqual(await response.json(), {
      credential: createCodexCredential(),
    })

...
test("managed ai codex oauth inference proxy treats blank base URL env values as unset", async () => {
  const previousInferenceBase = process.env.MANAGED_AI_CODEX_OAUTH_INFERENCE_BASE_URL
  const previousBase = process.env.MANAGED_AI_CODEX_OAUTH_BASE_URL
  ...
  try {
    ...
    if (previousInferenceBase === undefined) {
      delete process.env.MANAGED_AI_CODEX_OAUTH_INFERENCE_BASE_URL
    } else {
      process.env.MANAGED_AI_CODEX_OAUTH_INFERENCE_BASE_URL = previousInferenceBase
    }
    if (previousBase === undefined) {
      delete process.env.MANAGED_AI_CODEX_OAUTH_BASE_URL
    } else {
      process.env.MANAGED_AI_CODEX_OAUTH_BASE_URL = previousBase
    }
  }

...
test("DEN compatibility bootstrap creates the claim ledger and its indexes", () => {
  assert.match(bootstrapSource, /CREATE TABLE IF NOT EXISTS \\?`organization_trial_domain_claim\\?`/)
  assert.match(bootstrapSource, /\\?`claimed_at\\?` timestamp\(3\) NOT NULL DEFAULT \(now\(\)\)/)
  assert.match(
    bootstrapSource,

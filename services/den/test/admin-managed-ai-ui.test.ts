...
test("GET /admin/credentials serves the DEN admin shell with Codex inference controls and fallback provider credentials", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(html, /Sticky lease roster/i)
    assert.doesNotMatch(html, /id="session-list"/)
    assert.doesNotMatch(html, /id="session-detail"/)
    assert.doesNotMatch(html, /Session leases loading/i)
...
test("GET /admin/app.js uses DEN desktop auth and OpenAI OAuth credential routes", async () => {
  ...
  try {
    ...
    assert.match(script, /state\.credentials\.filter\(\(entry\) => !entry\.deletedAt\)\.length/)
    assert.match(script, /state\.credentials\.filter\(\(entry\) => !entry\.deletedAt && entry\.state !== "healthy"\)\.length/)
    assert.doesNotMatch(script, /sessions: \[\]/)
    assert.doesNotMatch(script, /selectedSessionId/)
    assert.doesNotMatch(script, /sessionList: document\.getElementById\("session-list"\)/)
    assert.doesNotMatch(script, /sessionDetail: document\.getElementById\("session-detail"\)/)
    ...
    assert.doesNotMatch(script, /els\.sessionList\.addEventListener/)

...
test("codex oauth inference proxy forwards tool-capable requests with server-side auth", async () => {
  const body = {
    model: "gpt-5.5",
    stream: false,
...
test("codex oauth inference proxy translates responses streaming to chat completion streaming", async () => {
  const transport = new CodexOAuthInferenceProxyTransport({
    baseUrl: "https://codex-inference.example.test",
    fetchImpl: async () =>
      new Response(
...
test("codex oauth inference proxy fails before upstream when auth json has no access token", async () => {
  const transport = new CodexOAuthInferenceProxyTransport({
    fetchImpl: async () => {

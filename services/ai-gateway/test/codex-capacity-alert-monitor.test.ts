...
} from "../src/alerts/codex-capacity-monitor.js";
...
test("Codex capacity monitor emails every configured admin recipient for urgent alerts", async () => {
  ...
  const result = await runCodexCapacityAlertMonitor({
    ...
    listAdminRecipients: async () => [" Admin-One@example.test ", "admin-two@example.test", "admin-one@example.test"],

...
test("organization billing and audit routes render real scoped loaders and actions", async () => {
  ...
  try {
    ...
    assert.match(html, /id="organization-billing-manual-unlimited"/)
    assert.match(html, /Unlimited trial \(no Veslo expiry, seat, or token cap\)/)
    assert.match(html, /id="organization-billing-unlimited-revoke"/)
    ...
    assert.match(script, /organizationBillingManualUnlimited/)
    assert.match(script, /revokeUnlimitedOrganizationTrial/)
    assert.match(script, /"Unlimited trial"/)
    assert.match(script, /"Unlimited"/)
    assert.match(script, /unlimited:\s*unlimitedTrial/)
    assert.match(script, /source:\s*unlimitedTrial \? "manual_trial" : null/)
    assert.match(script, /expiresAt:\s*unlimitedTrial \? null : manualAccessMode \? manualExpiresAt : null/)

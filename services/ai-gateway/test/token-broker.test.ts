...
import { DefaultTokenBroker, OpenAiOAuthRefreshError } from "../src/credentials/default-token-broker.js";
...
test("returns oauth access token for healthy openai binding", async () => {
  const secretStore = new EncryptedSecretStore("test_secret_key_32_bytes_minimum____", {
    secret_1: {
      kind: "openai_oauth",
      accessToken: "access_token_live",
  ...
  const auth = await broker.getUpstreamAuth({ bindingId: "binding_oauth" });

  assert.deepEqual(auth, {
    kind: "oauth",
    value: "access_token_live",
  });
});

...
test("deploy den workflow preserves hosted auth email env when repo inputs are blank", () => {
  ...
  assert.match(workflow, /if not resend_api_key:\s+resend_api_key = current_env_by_key\.get\("RESEND_API_KEY", ""\)/);
  assert.match(workflow, /if not auth_email_from:\s+auth_email_from = current_env_by_key\.get\("AUTH_EMAIL_FROM", ""\)/);
  assert.match(
    workflow,
